...
#An outlier line that will be added to other plots. It is 3.5 standard deviations above the mean.
...
#Although it won't be perfect, there is some correlation between LotFrontage and LotArea. By taking the ratio of the mean, this should provide a reasonable guess for what the LotFrontage is for the missing values.
...
#Here are the features for the one BsmtFinType2 that is missing its value, but relates to a basement. 
...
#Total the all the quality features to calculate the 'TotalQuality'.
...
```{r}
lmImportance <- lm(SalePrice ~ ., FinalTrain)
varImp(lmImportance)
```
...
partition <- createDataPartition(FinalTrain$SalePrice, p=0.75, list = FALSE)
...
tunes <- list(rqnc = caretModelSpec(method="rqnc",
                                    tuneGrid=data.frame(.lambda=c(0.000005),
                                                        .penalty = 'MCP')),
              rqlasso = caretModelSpec(method="rqlasso",
...
ensemble <- caretEnsemble(models, metric="RMSE",
                          trControl=trainControl(number=3, repeats = 1, classProbs=TRUE)
                         )
ensemble$error
summary(ensemble)
```
...
```{r}
preds <- data.frame(sapply(models, predict, newdata=FinalTest))
...
#rmse(testing$SalePrice,predsEnsemble)
